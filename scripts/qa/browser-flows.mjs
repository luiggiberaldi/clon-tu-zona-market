...
try{
  ...
  await scenario('Filters, price sort and clear use the published final price',async()=>{
    await goto(page,'/productos');await page.getByRole('spinbutton',{name:'Precio máximo en USD'}).fill('2');await page.getByLabel('Ordenar productos').selectOption('price_asc');await page.getByRole('button',{name:'Aplicar',exact:true}).click({timeout:180000});await page.waitForURL(/maxPrice=2/);
  ...
  await scenario('Product gallery, captured provenance, currency preference and guest cart',async()=>{
    ...
    await page.getByLabel('Moneda de precios',{exact:true}).selectOption('VES');await page.waitForFunction(()=>document.querySelector('main')?.innerText.includes('875'));
    await page.getByLabel('Moneda de precios',{exact:true}).selectOption('USD');
  ...
  await scenario('Address creation, edit, cancel and deletion maintain local data',async()=>{
    ...
    await form.locator('select').nth(0).selectOption({label:'Entrega simulada local'});await form.locator('select').nth(1).selectOption({label:'Sector de prueba · no se realiza envío'});await form.getByLabel('Dirección completa').fill('Casa de prueba avenida 123');await form.getByRole('button',{name:'Guardar dirección'}).click();await page.getByText('Dirección guardada.',{exact:true}).waitFor();
    ...
    await page.getByRole('button',{name:'Agregar dirección'}).click();form=page.locator('form').filter({has:page.getByRole('heading',{name:'Nueva dirección'})});await form.locator('select').nth(0).selectOption({label:'Entrega simulada local'});await form.locator('select').nth(1).selectOption({label:'Sector de prueba · no se realiza envío'});await form.getByLabel('Dirección completa').fill('Dirección secundaria para borrar');await form.getByRole('button',{name:'Guardar dirección'}).click();await page.getByText('Dirección guardada.',{exact:true}).waitFor();
  ...
  await scenario('Admin catalog creates, edits, uploads, adjusts and deactivates a local test product',async()=>{
    ...
    await form.locator('input[name="name"]').fill('Producto de prueba QA '+runId);await form.locator('input[name="sku"]').fill('QA-'+runId);await form.locator('textarea[name="description"]').fill('Registro creado únicamente durante las pruebas, no importado de la fuente.');await form.locator('input[name="price_usd"]').fill('2.50');await form.locator('input[name="stock_quantity"]').fill('12');await form.locator('input[name="min_stock"]').fill('0');await form.locator('select[name="category_id"]').selectOption(dataset.products[0].category_id);
  ...
  await scenario('Admin settings and coverage create/update/pause controls execute',async()=>{
    ...
    form=page.locator('form').filter({has:page.getByRole('heading',{name:'Crear ciudad',exact:true})});await form.locator('select[name="state"]').selectOption({label:'Estado de prueba '+runId});await form.locator('input[name="name"]').fill('Ciudad prueba '+runId);await form.locator('input[name="minimum"]').fill('0');await form.getByRole('button',{name:'Crear ciudad',exact:true}).click();await page.getByText('Cobertura guardada.',{exact:true}).waitFor();
    form=page.locator('form').filter({has:page.getByRole('heading',{name:'Crear sector',exact:true})});await form.locator('select[name="city"]').selectOption({label:'Ciudad prueba '+runId});await form.locator('input[name="name"]').fill('Sector prueba '+runId);await form.getByRole('button',{name:'Crear sector',exact:true}).click();await page.getByText('Cobertura guardada.',{exact:true}).waitFor();
  ...
  await scenario('Mobile navigation, coverage dialog and currency controls operate',async()=>{
    await page.setViewportSize({width:390,height:844});await goto(page,'/carabobo');assert(!await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),'Mobile overflow');await page.getByRole('button',{name:'Más opciones',exact:true}).click();await page.getByRole('dialog').waitFor();await page.getByRole('dialog').getByLabel('Moneda',{exact:true}).selectOption('VES');await page.keyboard.press('Escape');
    await page.locator('.mobile-bottom-nav').getByRole('button',{name:'Seleccionar zona de entrega',exact:true}).click();const zone=page.getByRole('dialog');await zone.getByLabel('Estado',{exact:true}).selectOption({label:'Carabobo'});await zone.getByLabel('Ciudad',{exact:true}).selectOption({label:'Entrega simulada local'});await zone.getByLabel('Urbanización o sector',{exact:true}).selectOption({label:'Sector de prueba · no se realiza envío'});await zone.getByRole('button',{name:'Confirmar mi zona'}).click();assert(await page.getByRole('dialog').count()===0,'Zone dialog did not close');await shot(page,'demo-real-mobile.png');

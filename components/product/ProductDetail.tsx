    ...
    <div className="storefront-container py-7 sm:py-10">
      ...
      <div className="grid min-w-0 grid-cols-1 items-start gap-8 md:grid-cols-2 lg:gap-16">
        ...
        <div className="min-w-0 space-y-5 pt-2">
          {product.category && <p className="text-xs font-bold uppercase tracking-[0.15em] text-primary">{product.category.name}</p>}
          ...
          <p className="flex items-center gap-2 text-sm text-primary"><PackageCheck size={17} />{product.stock_quantity > 0 ? `${product.stock_quantity} ${isDemoMode() ? 'en la copia local de prueba' : 'disponibles'}` : 'Producto agotado'}</p>
          ...
          {notice && <p role="status" className="rounded-lg bg-secondary p-3 text-sm">{notice} <Link href="/carrito" className="font-semibold text-primary underline">Ver carrito</Link></p>}

...
export function CartDrawer() {
  ...
  return <Dialog.Root open={isOpen} onOpenChange={toggle}><Dialog.Portal>
    ...
    <Dialog.Content className="fixed inset-y-0 right-0 z-[71] flex w-full max-w-md flex-col bg-background shadow-xl" onOpenAutoFocus={() => { previousFocus.current = document.activeElement instanceof HTMLElement ? document.activeElement : null; }} onCloseAutoFocus={event => { event.preventDefault(); previousFocus.current?.focus(); }}>
      ...
      {items.length > 0 && <div className="space-y-3 border-t p-4"><div className="flex justify-between text-sm"><span>Subtotal estimado</span><strong>{formatMoney(subtotal, 'USD')}</strong></div><p className="text-xs text-muted-foreground">Envío y disponibilidad se verifican en el checkout.</p><Button asChild className="w-full"><Link href="/carrito" onClick={close}>Ver carrito</Link></Button><Button asChild variant="secondary" className="w-full"><Link href="/checkout" onClick={close}>Continuar al checkout</Link></Button></div>}
    </Dialog.Content>

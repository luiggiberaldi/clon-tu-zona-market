...
export async function getCatalog(filters: ProductFilters = {}): Promise<{ data: ProductWithCategory[]; total: number; page: number; pageSize: number; hasMore: boolean; error?: string }> {
  ...
  try {
    ...
    if (error) return { ...empty, error: 'No pudimos cargar el catálogo. Comprueba la configuración y vuelve a intentarlo.' };
    return { data: (data || []) as unknown as ProductWithCategory[], total: count || 0, page, pageSize, hasMore: page * pageSize < (count || 0) };
  } catch { return { ...empty, error: 'La tienda está en configuración. El catálogo real estará disponible al conectar Supabase.' }; }
}
...
export async function getProduct(slug: string): Promise<ProductWithCategory | null> {
  if (!hasSupabaseConfig() && !isDemoMode()) return null;
